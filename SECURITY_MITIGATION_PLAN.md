...
**Last updated:** 2026-04-29 — PRs #2/#3/#4 merged; local follow-up closed the signed-policy write path, explicit cc_init_data claims, CAP/CLI runtime descriptor parity, attestation-proxy REPORT_DATA binding, tenant well-known routing, workload teardown, and unlock-mode receipt verification/persistence. The TEE channel now uses attestation-proxy dual listeners (internal HTTP 8081, external TLS 8443 via Service port 8081), and the CLI runs an attestation-pinned TLS handshake before claim/unlock/recover/change-password/auto-unlock sensitive payloads. External-track implementation has landed across `policy-templates`, `trustee`, `enclava-infra`, `attestation-proxy`, tenant manifests, `caddy-ingress`, and CAP follow-up wiring. Live Kata SNP LUKS smoke passes with the app-starts-first static wait-exec + long-running mounter-sidecar contract. cap-test01 is now cut over to the signed-policy/artifact path with `TRUSTEE_POLICY_READ_AVAILABLE=true`, signed-policy enforcement enabled in Trustee, a CAP-managed signed `resource-policy` artifact, and sidecar cosign verification over GitHub Actions OIDC-signed GHCR sidecar digests. The live CAP API, enclava-init, attestation-proxy, caddy-ingress, and Trustee KBS refs have been promoted off `ttl.sh` to repo-owned GHCR digests; Flux is deliberately suspended and scaled to zero during manual rollout validation. The CLI now bundles a signed development `platform-release.json` with the cap-test01 GHCR sidecar digests, signing-service pubkey, policy template text/hash, and a deterministic generator/checker. The legacy `flowforge-1` static tenant overlay has been retired from tenant GitOps, and stale legacy CAP/flowforge namespaces were deleted from the cluster. The policy-templates repository is public, but `ghcr.io/enclava-ai/policy-signing-service` is still not anonymously pullable; package visibility or a `read:packages` pull-secret remains required before replacing the off-cluster validation signing-service image.
...
| M1 — policy boundary intact | PARTIAL | cap-test01 is wired for and running the signed-policy/artifact path (`TRUSTEE_POLICY_READ_AVAILABLE=true`, Trustee policy/artifact URLs, signing-service pubkey envs, patched Trustee KBS with `require_signed_policy=true`, CAP-managed signed `resource-policy`). Tenant GitOps keeps the Trustee namespace but no longer owns the policy ConfigMap. cap-test01 now uses repo-owned GHCR image digests for CAP API, enclava-init, sidecars, and Trustee KBS, with sidecar cosign verification pinned to GitHub Actions OIDC identities. CLI now bundles a signed development platform-release artifact for the cap-test01 sidecar/template anchors. Blocked on durable signing-service deployment/key custody, policy-signing-service GHCR package visibility or pull credentials, production platform-release publishing, upstream-vs-fork ownership, and production legacy-policy template fate decisions. |
...
- #14 (CI/CD signing infra for D9) — **partially implemented, not production-final**. `../policy-templates/signing-service` exists and CI now publishes/signs `ghcr.io/enclava-ai/policy-signing-service`, but the container package is still not anonymously pullable even after the repository was made public. The active CAP deployment still points at `http://10.0.0.2:18080`; the signing-service manifest is not included by the cap-test01 kustomization and still needs image/package access, key custody, deployment target, and real genpolicy values. See `runbooks/external-tracks.md` item 1.
...
- Track 1 locally implemented: `../policy-templates/signing-service` now verifies descriptors/keyrings against a durable owner DB, renders/signs artifacts, has a genpolicy adapter, deterministic CE-v1/Ed25519 vector, receipt-gated Rego rules, docs, CI, tests, clippy, and a GHCR publish/sign workflow. CAP consumer wiring exists, and CAP now bundles a signed development `platform-release.json` with cap-test01 sidecar/template anchors. Remaining work: make the policy-signing-service GHCR package public or provide package-read credentials, real `genpolicy` binary/settings pin, key custody, deployment, production release publishing, and CLI descriptor/keyring upload.
...
- Former rollout caveat resolved for cap-test01: the live CAP/API, attestation-proxy, caddy-ingress, Trustee KBS, and enclava-init refs have been promoted from `ttl.sh` validation refs to repo-owned GHCR digests, with sidecar images signed by their repo workflows. The signing-service image is published and signed from `../policy-templates`, but GHCR still rejects anonymous pulls for `ghcr.io/enclava-ai/policy-signing-service`; cap-test01 continues to point at the off-cluster validation signing service until package visibility or pull credentials are fixed.
- Main rollout blocker: production release publishing and platform-release distribution. CAP now has a signed development platform-release artifact carrying cap-test01 sidecar image digests, signing-service pubkey, policy template text/hash, and genpolicy string, but the production path still needs release-root custody, real genpolicy pinning, signing-service deployment/key custody, and full deploy-flow consumption before M1/M4/M5 can be claimed broadly. Flux is suspended/scaled down while this manual promotion is validated.
