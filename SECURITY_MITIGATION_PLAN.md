...
**Last updated:** 2026-04-28 — PRs #2/#3/#4 merged; local follow-up closed the signed-policy write path, explicit cc_init_data claims, CAP/CLI runtime descriptor parity, attestation-proxy REPORT_DATA binding, tenant well-known routing, workload teardown, and unlock-mode receipt verification/persistence. The TEE channel now uses attestation-proxy dual listeners (internal HTTP 8081, external TLS 8443 via Service port 8081), and the CLI runs an attestation-pinned TLS handshake before claim/unlock/recover/change-password/auto-unlock sensitive payloads. External-track implementation has landed across `policy-templates`, `trustee`, `enclava-infra`, `attestation-proxy`, tenant manifests, `caddy-ingress`, and CAP follow-up wiring. Live Kata SNP LUKS smoke passes with the app-starts-first static wait-exec + long-running mounter-sidecar contract. cap-test01 is now cut over to the signed-policy/artifact path with `TRUSTEE_POLICY_READ_AVAILABLE=true`, signed-policy enforcement enabled in Trustee, a CAP-managed signed `resource-policy` artifact, and sidecar cosign verification via public-key policy. The legacy `flowforge-1` static tenant overlay has been retired from tenant GitOps, and stale legacy CAP/flowforge namespaces were deleted from the cluster.
...
|---|---|---|---|
| 0 | PARTIAL | env-var gates, SSRF-defended outbound HTTP, SA automount disabled, JWT iss/aud/typ/jti, webhook MAC + replay (`processed_webhooks`), server-side billing intent fields, NIP-98 payload-tag helper, trusted-proxy rate limiter, env-driven CORS allowlist, tenant TLS-ALPN-only Caddyfile, no tenant Cloudflare secret path, no-DNS-plugin `caddy-ingress`, tenant-manifest template cutover, CT monitoring script/runbook, legacy `flowforge-1` overlay retired, stale legacy CAP/flowforge namespaces deleted | **Production cutover ops:** publish CAA records (`accounturi` + `validationmethods=tls-alpn-01`), deploy durable no-DNS-plugin caddy-ingress images, and schedule CT monitoring. |
| 1 | DONE | Migrations 0010–0020, CE-v1 helpers (D11), validators, hostname helpers, identity hash migrated to CE-v1 | — |
| 2 | PARTIAL | Local sibling `../policy-templates` now has a v1 Rust signing service: CE-v1 bytes, descriptor/keyring canonicalization, durable SQLite owner store, `/sign`, signed metadata, artifact verification, genpolicy adapter, deterministic vector, docs, CI. CAP's Kata agent policy is fail-closed. CAP deploy now accepts optional `customer_descriptor_blob` + `org_keyring_blob`, calls `PLATFORM_SIGNING_SERVICE_URL`, verifies the returned artifact signature with configured `SIGNING_SERVICE_PUBKEY_HEX`, persists descriptor/keyring/policy artifacts in `workload_artifacts`, and carries explicit descriptor/keyring/identity claims into cc_init_data. CLI deploy can now generate/sign descriptor + keyring blobs when all exact chain anchors are locally available and its runtime spec mirrors the CAP-rendered wait-exec app container contract. | Still left: publish bundled platform release data so CLI does not need env-backed expected hashes; pick key custody/deployment target; pin/run real `genpolicy`; publish release verify pubkey. |
| 3 | PARTIAL | Local sibling `../trustee` now has SNP `init_data_hash`, signed-policy write/evaluation enforcement, workload-attested policy body read, body/receipt policy inputs, required `If-None-Match`/`If-Match` preconditions, insert-if-absent storage backends, and `POST /kbs/v0/attestation/verify`. CAP API has workload-attested `GET /api/v1/workload/artifacts` with Trustee callback validation and init-data-hash cross-check. Signed deployment apply writes the verified signed policy artifact envelope to Trustee and skips local Rego reconciliation. cap-test01 runs the patched Trustee KBS image with `require_signed_policy=true`; an unsigned raw Rego negative test fails closed before KBS serves policy. | Decide upstream-vs-fork; publish a durable non-`ttl.sh` Trustee image; add per-resource version/ETag CAS after v1; classify legacy/operator policy lines into the signing-service template or documented deprecations before broader production rollout. |
...
| M1 — policy boundary intact | PARTIAL | cap-test01 is wired for and running the signed-policy/artifact path (`TRUSTEE_POLICY_READ_AVAILABLE=true`, Trustee policy/artifact URLs, signing-service pubkey envs, patched Trustee KBS with `require_signed_policy=true`, CAP-managed signed `resource-policy`). Tenant GitOps keeps the Trustee namespace but no longer owns the policy ConfigMap. Blocked on durable signing-service deployment/key custody, platform-release publishing, durable non-`ttl.sh` image provenance, upstream-vs-fork ownership, and production legacy-policy template fate decisions. |
| M2 — operator out of seed loop | PARTIAL | Receipt signer + Trustee receipt inputs + receipt-gated template rules exist locally; CAP no longer ships `kbs-resource-writer`, and the active cap-test01 kustomization removed the writer plus token Secret. Proxy write/rekey/delete uses workload-resource preconditions and receipt envelopes; cap-test01 validates the no-writer path. Blocked on durable non-`ttl.sh` Trustee/proxy image provenance and storage-level versioned CAS hardening. |
| M3 — tenant-bound image trust | **DONE** | Phase 9 in PR #1 |
| M4 — CLI proves real TEE | PARTIAL | Phase 5 code path, REPORT_DATA binding, and the live CLI sensitive-payload path now use attestation-pinned TLS and receipt-key binding. Still blocked on bundled platform-release anchors, full raw AMD SNP report/VCEK validation, keyring UX completion, and durable production Trustee/proxy/API rollout. |
...
- No open CAP PRs remain; the current unmerged work is local cross-repo follow-up described below.
...
- #14 (CI/CD signing infra for D9) — **partially implemented, not production-final**. `../policy-templates/signing-service` exists and the cap-test01 ops overlay has a signing-service manifest scaffold, but the active CAP deployment still points at `http://10.0.0.2:18080`; the signing-service manifest is not included by the cap-test01 kustomization and still has placeholder image/key/genpolicy values. See `runbooks/external-tracks.md` item 1.
...
- Track 1 locally implemented: `../policy-templates/signing-service` now verifies descriptors/keyrings against a durable owner DB, renders/signs artifacts, has a genpolicy adapter, deterministic CE-v1/Ed25519 vector, receipt-gated Rego rules, docs, CI, tests, and clippy. CAP consumer wiring exists. Remaining work: real `genpolicy` binary/settings pin, key custody, deployment, release publishing, and CLI descriptor/keyring upload.
- Track 2 implemented and live-validated in cap-test01: `../trustee` now has signed-policy enforcement, workload-attested policy body read, body/receipt inputs, attestation verify callback, SNP `init_data_hash`, required write/delete preconditions, and insert-if-absent storage writes. `key-value-storage` tests pass locally; KBS/verifier focused tests are blocked on this workstation by missing Intel DCAP quote verifier headers (`sgx_dcap_quoteverify.h`). cap-test01 runs the patched KBS image with signed-policy enforcement. Remaining work: upstream-vs-fork decision, durable production image publishing, and per-resource version/ETag CAS.
...
- Tenant GitOps keeps `infra/trustee-kbs-policy` only for namespace ownership; that sub-kustomization is ConfigMap-free and no longer applies the historical Trustee `resource-policy`. CAP now owns the signed policy write. This is the live Trustee signed-policy cutover on the manifest side; full M1 remains blocked by durable release/provenance, key custody, upstream/fork ownership, and legacy-policy fate decisions.
...
- Temporary rollout caveat: the live CAP/API, attestation-proxy, caddy-ingress, Trustee KBS, and enclava-init image refs are validation refs (`ttl.sh/...@sha256:...` where applicable), and the signing-service manifest is only a scaffold with placeholder digest/key/genpolicy values. These unblock validation but are not durable release provenance.
- Main rollout blocker: release publishing and platform-release distribution. cap-test01 sidecar provenance validation is active via public-key cosign policy over signed digest-pinned sidecar images, but platform-release data still must carry durable sidecar image digests, signing-service pubkey, policy template text/hash, and genpolicy pin before M1/M4/M5 can be claimed broadly.
...
| Kata agent policy is fail-open today | `cap/crates/enclava-engine/src/manifest/cc_init_data.rs:45-55` |
...
| Tenant Caddy uses TLS-ALPN-01 without a Cloudflare DNS plugin or tenant token | `caddy-ingress/Dockerfile`; `cap/crates/enclava-engine/src/manifest/ingress.rs`; `cap/crates/enclava-engine/src/manifest/containers.rs`; `enclava-tenant-manifests/components/templates/confidential-workload/*` |
