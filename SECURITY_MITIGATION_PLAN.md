...
| 2 | PARTIAL | Local sibling `../policy-templates` now has a v1 Rust signing service: CE-v1 bytes, descriptor/keyring canonicalization, durable SQLite owner store, `/sign`, signed metadata, artifact verification, deterministic vector, docs, CI, and a real Kata `genpolicy` adapter. The signing-service image bakes checksum-verified `kata-tools-static-3.28.0-amd64.tar.zst`, passes explicit `rules.rego`/settings paths, refuses unpinned genpolicy labels at startup, and runs `genpolicy` before signing the KBS artifact. CAP's current Kata agent policy is fail-closed but still generated by CAP; the genpolicy output is a signing gate today, not yet the `cc_init_data` policy body. CAP deploy and unlock-mode redeploy paths require `customer_descriptor_blob` + `org_keyring_blob` whenever signed-policy/signing-service infrastructure is configured, call `PLATFORM_SIGNING_SERVICE_URL`, verify the returned artifact signature with configured `SIGNING_SERVICE_PUBKEY_HEX`, persist descriptor/keyring/policy artifacts in `workload_artifacts`, and carry explicit descriptor/keyring/identity claims into cc_init_data. CLI deploy now requires a digest-pinned `--image`; auto-unlock enable/disable also require a digest-pinned `--image` so the mode-transition redeploy has a fresh signed descriptor. CLI has a signed bundled development `platform-release.json` plus generator/checker carrying the cap-test01 GHCR sidecar digests, signing-service pubkey, policy template text/hash, and Kata 3.28.0 genpolicy pin. CAP API now consumes that signed release in signed-policy mode and refuses env drift for the signing-service URL/pubkeys and sidecar image anchors. The off-cluster validation signing-service now runs the public, keyless-signed GHCR digest. | Still left: wire generated Kata agent policy into `cc_init_data` and the CLI/API descriptor hash flow; production platform-release signing-root custody and release process; pick durable signing-service key custody/deployment target or replace platform policy signing with customer/CI-signed policy artifacts; publish production release verify pubkey. |
...
- Main rollout blocker: production release publishing and platform-release distribution. CAP now has a signed development platform-release artifact carrying cap-test01 sidecar image digests, signing-service pubkey, policy template text/hash, and Kata 3.28.0 genpolicy pin, and CAP API consumes those signed anchors in signed-policy mode. The production path still needs generated-agent-policy wiring, release-root custody, signing-service deployment/key custody, and full production release publishing before M1/M4/M5 can be claimed broadly. Flux is suspended/scaled down while this manual promotion is validated.
