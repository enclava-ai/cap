...
**Last updated:** 2026-04-29 — PRs #2/#3/#4 merged; local follow-up closed the signed-policy write path, explicit cc_init_data claims, CAP/CLI runtime descriptor parity, attestation-proxy REPORT_DATA binding, tenant well-known routing, workload teardown, and unlock-mode receipt verification/persistence. The TEE channel now uses attestation-proxy dual listeners (internal HTTP 8081, external TLS 8443 via Service port 8081), and the CLI runs an attestation-pinned TLS handshake before claim/unlock/recover/change-password/auto-unlock sensitive payloads. External-track implementation has landed across `policy-templates`, `trustee`, `enclava-infra`, `attestation-proxy`, tenant manifests, `caddy-ingress`, and CAP follow-up wiring. Live Kata SNP LUKS smoke passes with the app-starts-first static wait-exec + long-running mounter-sidecar contract. cap-test01 is now cut over to the signed-policy/artifact path with `TRUSTEE_POLICY_READ_AVAILABLE=true`, signed-policy enforcement enabled in Trustee, a CAP-managed signed `resource-policy` artifact, and sidecar cosign verification over GitHub Actions OIDC-signed GHCR sidecar digests. The live CAP API is now `ghcr.io/enclava-ai/enclava-api@sha256:1dd80a03b0bff16605a613b02d2f35f8c953b0daed2eb13212b7f5c15193f67d`; enclava-init, attestation-proxy, caddy-ingress, Trustee KBS, and the off-cluster validation signing service refs have also been promoted off `ttl.sh` to repo-owned GHCR digests. Flux is deliberately suspended and scaled to zero during manual rollout validation. The CLI now bundles a signed development `platform-release.json` with the cap-test01 GHCR sidecar digests, signing-service pubkey, policy template text/hash, and a deterministic generator/checker. `enclava deploy` now requires a digest-pinned image and always sends customer-signed descriptor/keyring blobs; auto-unlock mode transitions also require a digest-pinned image and send fresh descriptor/keyring blobs for the resulting redeploy. CAP rejects unsigned deploys and unlock-mode redeploys whenever signed-policy/signing-service infrastructure is configured. The legacy `flowforge-1` static tenant overlay has been retired from tenant GitOps, stale legacy CAP/flowforge namespaces were deleted from the cluster, and the old scaled-to-zero `flowforge-ownership-test-ot-2` resources were removed from `default`. The policy-signing-service GHCR package is public/pullable, its keyless signature verifies against the `enclava-ai/policy-templates` GitHub Actions workflow identity, and the validation service at `http://10.0.0.2:18080` is now backed by `ghcr.io/enclava-ai/policy-signing-service@sha256:fe2900904f85d2cb58c6577b5bd86c2580e77d8962cfee9539c81e2e661e02eb`.
...
| 2 | PARTIAL | Local sibling `../policy-templates` now has a v1 Rust signing service: CE-v1 bytes, descriptor/keyring canonicalization, durable SQLite owner store, `/sign`, signed metadata, artifact verification, genpolicy adapter, deterministic vector, docs, CI. CAP's Kata agent policy is fail-closed. CAP deploy and unlock-mode redeploy paths require `customer_descriptor_blob` + `org_keyring_blob` whenever signed-policy/signing-service infrastructure is configured, call `PLATFORM_SIGNING_SERVICE_URL`, verify the returned artifact signature with configured `SIGNING_SERVICE_PUBKEY_HEX`, persist descriptor/keyring/policy artifacts in `workload_artifacts`, and carry explicit descriptor/keyring/identity claims into cc_init_data. CLI deploy now requires a digest-pinned `--image`; auto-unlock enable/disable also require a digest-pinned `--image` so the mode-transition redeploy has a fresh signed descriptor. CLI has a signed bundled development `platform-release.json` plus generator/checker carrying the cap-test01 GHCR sidecar digests, signing-service pubkey, policy template text/hash, and genpolicy string. The off-cluster validation signing-service now runs the public, keyless-signed GHCR digest. | Still left: production platform-release signing-root custody and release process; pick durable signing-service key custody/deployment target or replace platform policy signing with customer/CI-signed policy artifacts; pin/run real `genpolicy`; publish production release verify pubkey. |
...
| M1 — policy boundary intact | PARTIAL | cap-test01 is wired for and running the signed-policy/artifact path (`TRUSTEE_POLICY_READ_AVAILABLE=true`, Trustee policy/artifact URLs, signing-service pubkey envs, patched Trustee KBS with `require_signed_policy=true`, CAP-managed signed `resource-policy`). Tenant GitOps keeps the Trustee namespace but no longer owns the policy ConfigMap. cap-test01 now uses repo-owned GHCR image digests for CAP API, enclava-init, sidecars, Trustee KBS, and the off-cluster validation signing service, with sidecar/signing-service cosign verification pinned to GitHub Actions OIDC identities. CLI now bundles a signed development platform-release artifact for the cap-test01 sidecar/template anchors. Blocked on durable signing-service key custody/deployment target, production platform-release publishing, upstream-vs-fork ownership, and production legacy-policy template fate decisions. |
...
- #14 (CI/CD signing infra for D9) — **partially implemented, not production-final**. `../policy-templates/signing-service` exists; CI publishes/signs public `ghcr.io/enclava-ai/policy-signing-service`, and the off-cluster validation service at `http://10.0.0.2:18080` now runs the verified GHCR digest. The signing-service manifest is still not included by the cap-test01 kustomization because durable key custody/deployment target and real genpolicy values remain unresolved. See `runbooks/external-tracks.md` item 1.
...
- Track 1 locally implemented: `../policy-templates/signing-service` now verifies descriptors/keyrings against a durable owner DB, renders/signs artifacts, has a genpolicy adapter, deterministic CE-v1/Ed25519 vector, receipt-gated Rego rules, docs, CI, tests, clippy, and a GHCR publish/sign workflow. CAP consumer wiring exists, CAP now bundles a signed development `platform-release.json` with cap-test01 sidecar/template anchors, and the off-cluster validation signing service is promoted to the public signed GHCR digest. Remaining work: real `genpolicy` binary/settings pin, durable key custody, production deployment target, production release publishing, and CLI descriptor/keyring upload.
