...
- Former rollout caveat resolved for cap-test01: the live CAP/API, attestation-proxy, caddy-ingress, Trustee KBS, and enclava-init refs have been promoted from `ttl.sh` validation refs to repo-owned GHCR digests, with sidecar images signed by their repo workflows. The signing-service manifest is still only a scaffold with placeholder digest/key/genpolicy values; cap-test01 continues to point at the off-cluster validation signing service.
