...
| M1 — policy boundary intact | PARTIAL | cap-test01 is wired for and running the signed-policy/artifact path (`TRUSTEE_POLICY_READ_AVAILABLE=true`, Trustee policy/artifact URLs, signing-service pubkey envs, patched Trustee KBS with `require_signed_policy=true`, CAP-managed signed `resource-policy`). Tenant GitOps keeps the Trustee namespace but no longer owns the policy ConfigMap. cap-test01 now uses repo-owned GHCR image digests for CAP API, enclava-init, sidecars, and Trustee KBS, with sidecar cosign verification pinned to GitHub Actions OIDC identities. Blocked on durable signing-service deployment/key custody, platform-release publishing, upstream-vs-fork ownership, and production legacy-policy template fate decisions. |
...
- Main rollout blocker: release publishing and platform-release distribution. cap-test01 sidecar provenance validation is active via GitHub Actions OIDC cosign policy over signed GHCR digest-pinned sidecar images, but platform-release data still must carry sidecar image digests, signing-service pubkey, policy template text/hash, and genpolicy pin before M1/M4/M5 can be claimed broadly. Flux is suspended/scaled down while this manual promotion is validated.
